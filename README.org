...
Aquí configuro el programa offlineimap para poder descargarme los correos del servidor correspondiente. Actualmente lo combino con notmuch y el cliente para emacs de notmuch, y así puedo leer el correo en Emacs. La configuración necesaria es tener el .offlineimaprc creo que en ~/.offlineimaprc. Luego ejecutar notmuch y configurarlo con el asistente, y por último hay que ejecutar "notmuch new" cada vez que offlineimap descargue correo. Para ello se puede por ejemplo crear un script en ~/bin/mail-notiffication, poner dentro "notmuch new" y luego hacerle "chmod +x". La línea de .offlineimaprc de postcript apunta al script.
